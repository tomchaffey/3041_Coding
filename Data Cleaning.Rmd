...
### Lock 9 total cyanobacteria arima 
```{r}
head(L9sum)
...
cell_test = ts(log10(l9_test$CELLCOUNT), 
               frequency = 45,
               start= c(2015, 06))

l9.arima <- auto.arima(log10(L9sum$CELLCOUNT),
                   xreg = L9sum$Value) # run arima
l9.fc <- forecast(l9.arima, 
                  xreg = rep(10,6), # not sure what rep does...
                  h = 52) # forecast arima
plot(l9.fc) # plot forecast 
...
autoplot(train.fc) + autolayer(cell_test)
