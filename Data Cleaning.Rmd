...
autoplot(lock_cellcount) +
  labs(x = "Date",
       y= "Cellcount (cells/mL)",
       title = "Total Cellcount at Lock 9")+
  theme_minimal()
autoplot(lock_discharge) +
  labs(x = "Date",
       y = "Dischage [cumec]",
       title = "Discharge at Lock 9") +
  theme_minimal()

autoplot(lock_logcellcount) +
  labs(x = "Date",
       y = "Log10 Cellcount (cells/mL)",
       title = "Log Total Cellcount at Lock 9") +
  stat_smooth(method = "lm") +
  theme_minimal() +
  annotate("text", x = 2015, y = 2 , label = "R^2 = , p < 0.001")
...
log_lock.lm <- lm(log10(L9sum$CELLCOUNT)~L9sum$Group.date)
log_lock.lm
...
swan_sum1 <- swan_sum %>%
  mutate(diff_cell = diff(CELLCOUNT))

swan_sum$diff_cell <- diff(swan_sum$CELLCOUNT)

s_cell.ts <- ts(swan_sum$CELLCOUNT,
                frequency = 45,
                start = c(2016))

autoplot(diff(s_cell.ts)) +
  labs(x = "Date",
       y = "Differenced Cellcount (cells/mL)",
       title = "Differenced Cellcount at Swan Hill") + 
  theme_minimal() 
autoplot(acf(diff(s_cell.ts))) +
           theme_minimal() +
           labs(title = "ACF Differenced Cellcount")
autoplot(pacf(diff(s_cell.ts))) +
  theme_minimal() +
  labs(title = "PACF Differenced Cellcount")

autoplot(s_cell.ts)

diff(s_cell.ts) %>% autoplot() +
  theme_minimal() +
  labs(x = "Date",
       y = "Differenced Cellcount",
       title = "Differenced Cellcount at Swan Hill")
