...
autoplot(yarra.fc) + autolayer(Yarrawonga_Observed)

acf(yarra_sum$CELLCOUNT)
pacf(yarra_sum$CELLCOUNT)

fitx <- auto.arima(yarra_train$CELLCOUNT)
plot(forecast(fitx))
...
autoplot(swanhill.fc) + autolayer(SwanHill_Observed)

acf(swan_sum$CELLCOUNT)
pacf(swan_sum$CELLCOUNT)

fitx <- auto.arima(swan_train$CELLCOUNT)
plot(forecast(fitx))
```
Heywoods
```{r}
...
h_cell.train <- ts(log10(hey_train$CELLCOUNT),
                   frequency = 45,
                   )
h_dc.test <- ts(hey_test$Value,
                   frequency = 45,
                   )
Heywoods_Observed <- ts(log10(hey_test$CELLCOUNT),
                   frequency = 45,
                   start = c(3.8))
...
autoplot(heywoods.fc) + autolayer(Heywoods_Observed)
...
m_cell.train <- ts(log10(merb_train$CELLCOUNT),
                   frequency = 45,
                   )
m_dc.test <- ts(merb_test$Value,
                   frequency = 45,
                   )
Merbein_Observed <- ts(log10(merb_test$CELLCOUNT),
                   frequency = 45,
                   start =c(3.54))
...
autoplot(merbein.fc) + autolayer(Merbein_Observed)
...
autoplot(burtundy.fc) + autolayer(Burtundy_Observed)
...
autoplot(Balranald.fc) + autolayer(Balranald_Observed)
