...
Here is the final script for loading, cleaning and ARIMA for PMP data and Water data online discharge data.
...
Loading and collating PMP data from only the AWQC data set from the PMP.
...
```
Combined data frame for both Phytoplankton cell-count and discharge.
...
Summary statistics showing prominent taxa within the set. Not the broad differences in cellcount amongst taxa, and the different reporting of taxa, from species to family. 
...
```
More summary statisitics for the combined data frame, note the number of unique taxa amongst sites and total cell-counts per taxon. 

# Subsetting out df's for analysis

Code for subsetting the combined data frame by site, including time series plots for both prominent taxa and discharge. Prominent taxa include; Aulacoseira, Anabaena, and total blue-green algae
...
We will be utilising ARIMA modelling to analyse our data set. Firstly, by splitting our site based data in training:testing data, approximately 80:20. ARIMA model is trainned with trainning data, and the forecasts are tested against observed results. This allows us to visuale the strength of our model, the signals produced in conjunction with various summary statistics of the model errors. 
...
lock_cellcount <- xts(L9sum$CELLCOUNT, order.by = L9sum$Group.date)
lock_logcellcount <- xts(log10(L9sum$CELLCOUNT), order.by = L9sum$Group.date)
lock_discharge <- xts(L9sum$Value, order.by = L9sum$Group.date)


l9_cell.ts <- ts(L9sum$CELLCOUNT,
                 frequency = 45,
                 start = c(1997))

autoplot(acf(diff(l9_cell.ts))) +
  theme_minimal() +
  labs(title = "ACF Differenced Cellcount Lock 9")
autoplot(pacf(diff(l9_cell.ts))) +
  theme_minimal() +
  labs(title = "PACF Differenced Cellcount Lock 9")


### making xregs for l9 discharge ...

l9_train = L9sum[1:801,] # training set
l9_test = L9sum[802:1003,] # testing set
discharge.train = ts(l9_train$Value, 
                     frequency = 45,
                     start = c(1997))
cell_train = ts(l9_train$CELLCOUNT,
                frequency = 45,
                start = c(1997))
discharge.test = ts(l9_test$Value,
                    frequency = 45,
                    start = c(2014.9)) # test and train should be 80/20 of the vector 
Observed_Results = ts(l9_test$CELLCOUNT, 
               frequency = 45,
               start = c(2014.9))
fitrain <- auto.arima(cell_train, 
                      xreg = discharge.train) #train ARIMA model with x-regs for lock 9
train.fc <- forecast(fitrain,
                     xreg = discharge.test) #run hindcasts using future x-regs
plot(train.fc) #show observed/forecasted results
# with observed 
autoplot(train.fc) + 
...
summary(train.fc) #summary statistics for ARIMA model
fitrain
summary(fitrain)
...
#given the large ts., logged results can show broad trends in the data. Running ARIMA for the following
log_discharge.train = ts(l9_train$Value, 
                     frequency = 45,
                     start = c(1997))
log_cell_train = ts(log10(l9_train$CELLCOUNT),
                frequency = 45,
                start = c(1997))
log_discharge.test = ts(l9_test$Value,
                    frequency = 45,
                    start = c(2014.9)) # test and train should be 80/20 of the vector 
Logged_Observed_Results = ts(log10(l9_test$CELLCOUNT), 
               frequency = 45,
               start = c(2014.9))
log_fitrain <- auto.arima(log_cell_train, 
                      xreg = log_discharge.train)
log_train.fc <- forecast(log_fitrain,
                     xreg = log_discharge.test)

autoplot(log_train.fc) + 
  autolayer(Logged_Observed_Results) +
  theme_minimal() +
  labs(x = "Date",
       y = "Log10 Cellcount (cells/mL)",
       title = "ARIMA (1,1,2)(0,0,1)[45] Forecast at Lock 9",
       color = "")

###Plots

autoplot(train.fc) +  # plot of observed results and forecasts
  autolayer(Observed_Results) +
  theme_minimal() +
  labs(x = "Date",
       y = "Cellcount (cells/mL)",
       title = "ARIMA errors (1,1,02)(0,0,2)[45] Forecast at Lock 9",
       color = "")

autoplot(train.fc) + #plot of previous observed results and forecasts, for comparison 
  theme_minimal() +
  labs(x = "Date",
       y = "Cellcount (cells/mL)",
       title = "ARIMA errors (1,1,2)(0,0,2)[45] Forecast at Lock 9",
       color = "")

autoplot(lock_cellcount) + # cleaner ts plot of cellcount
  labs(x = "Date",
       y= "Cellcount (cells/mL)",
       title = "Total Cellcount at Lock 9")+
  theme_minimal()
autoplot(lock_discharge) + # cleaner ts. for discharge
  labs(x = "Date",
       y = "Dischage [cumec]",
       title = "Discharge at Lock 9") +
  theme_minimal()


log_lock.lm <- lm(log10(L9sum$CELLCOUNT)~L9sum$Group.date) # run linear model on  logged data to see broad increases/decreases in cellcount
log_lock.lm

plot(log_lock.lm)
summary(log_lock.lm)

plot(log10(L9sum$CELLCOUNT)~L9sum$Group.date)
abline(log_lock.lm)
...
This same pattern is used for the rest of the sites. 
...
y_dc.train <- ts(yarra_train$Value,
                 frequency = 45,
                 start = c(2016,7))
y_cell.train <- ts(yarra_train$CELLCOUNT,
                   frequency = 45,
                   start = c(2016,7))
y_dc.test <- ts(yarra_test$Value,
                   frequency = 45,
                   start = c(2019,1))
Yarrawonga_Observed <- ts(yarra_test$CELLCOUNT,
                   frequency = 45,
                   start = c(2018.8))
...
autoplot(yarra.fc) + 
  ...
  labs(x = "Date",
       y = "Cellcount (cells/mL)",
       title ="ARIMA (0,1,1) Forecast at Yarrawonga",
       color = "") +
  annotate("text", x = 2016.5, y = -10000, label = "AICc=88.71")
...
s_cell.ts <- ts(swan_sum$CELLCOUNT, # time series of cellcount for acf/pacf plots
                frequency = 45,
                start = c(2016))
autoplot(diff(s_cell.ts)) + # plots for differenced cellcount, auto.arima does this, making plots purely for presentation
  labs(x = "Date",
       y = "Differenced Cellcount (cells/mL)",
       title = "Differenced Cellcount at Swan Hill") + 
  theme_minimal() 
autoplot(acf(diff(s_cell.ts))) + #acf plots
           theme_minimal() +
           labs(title = "ACF Differenced Cellcount")
autoplot(pacf(diff(s_cell.ts))) + #pacf plots
  theme_minimal() +
  labs(title = "PACF Differenced Cellcount")
...
swan_discharge <- xts(swan_sum$Value, order.by = swan_sum$Group.date)
swan_cellcount <- xts(swan_sum$CELLCOUNT, order.by = swan_sum$Group.date)
...
s_dc.test <- ts(swan_test$Value,
                   frequency = 45,
                   start =c(2018.8))
Observed.Results <- ts(swan_test$CELLCOUNT,
                   frequency = 45,
                   start = c(2018.8))
...
autoplot(swanhill.fc) + 
  autolayer(`"Observed Results"`) + 
  theme_minimal() +
  labs(x = "Date",
       y = "Cellcount",
       title = "ARIMA (0,1,1) Forecasts at Swan Hill",
       color = "") +
  annotate("text", x = 2016.5, y = -110000, label = "AICc = 3040.04")

autoplot(swanhill.fc) + 
  autolayer(`"Observed Results"`) + 
  theme_minimal() +
  labs(x = "Date",
       y = "Log 10 Cellcount (cells/mL)",
       title = "ARIMA (0,1,1) Forecasts at Swan Hill",
       color = "") +
  scale_y_log10() +
  annotate("text", x = 2016.5, y = -110000, label = "AICc = 3040.04")
...
autoplot(swan_discharge) +
  theme_minimal() +
  labs(x = "Date",
         y = "Discharge [cumec]",
         title = "Discharge at Swan Hill")
autoplot(swan_cellcount) +
  theme_minimal() +
  labs(x = "Date",
         y = "Cellcount (cells/mL)",
         title = "Total Cellcount at Swan Hill")

summary(swanhill.fc)

summary(fit_swanhill)
...
autoplot(heywoods.fc) + 
  ...
  labs(x = "Date",
   ...
   annotate("text", x = 2019, y = 150000, label = "AICc=2916.95")
...
autoplot(merbein.fc) + 
  ...
  labs(x = "Date",
       y = "Cellcount (cells/mL)",
       title = "ARIMA (0,1,2) Forecasts at Merbein",
       color = "") +
  annotate("text", x = 2018.5, y = 150000, label = "AICc=2648.06")
...
autoplot(burtundy.fc) + 
  ...
  labs(x = "Date",
       ...
       color = "") +
    annotate("text", x = 2016.5, y = 1000000, label = "AICc=3141.35")
...
autoplot(Balranald.fc) + 
  ...
  labs(x = "Date",
       ...
       color = "") +
      annotate("text", x = 2016.5, y = -100000, label = "AICc=3005.31")
